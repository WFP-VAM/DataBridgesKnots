...
def test_get_variable_labels_empty_label():
...
def test_get_variable_labels_duplicate_names():
...
def test_get_variable_labels_empty_dataframe():
    df = pd.DataFrame(columns=["name", "label"])
    result = get_variable_labels(df)
    assert result == {}


def test_get_variable_labels_empty_string():
