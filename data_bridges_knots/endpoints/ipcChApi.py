...
class IpcchApi:
    ...
    def get_ipc_and_equivalent_latest_peaks(self, iso3: str | None = None, page: int | None = 1) -> pd.DataFrame:
        with data_bridges_client.ApiClient(self.configuration) as api_client:
        # Create an instance of the API class
        api_instance = data_bridges_client.IpcchApi(api_client)
        ...
        try:
            ...
            logger.info("The response of IpcchApi->ipcch_ipcch_and_equivalent_latest_peaks_get:\n")
            df = pd.DataFrame([item.to_dict() for item in api_response.items])
            return df
        except Exception as e:
            logger.error("Exception when calling IpcchApi->ipcch_ipcch_and_equivalent_latest_peaks_get: %s\n" % e)
            raise e

    def get_ipc_and_equivalent_most_recent(self, iso3: str | None = None, page: int | None = 1) -> pd.DataFrame:
        with data_bridges_client.ApiClient(self.configuration) as api_client:
            # Create an instance of the API class
            api_instance = data_bridges_client.IpcchApi(api_client)
            env = self.env # str | Environment.   * `prod` - api.vam.wfp.org   * `dev` - dev.api.vam.wfp.org (optional)
            ...
            try:
                ...
                logger.info("The response of IpcchApi->ipcch_ipcch_and_equivalent_most_recent_get:\n")
                df = pd.DataFrame([item.to_dict() for item in api_response.items])
                return df
            except Exception as e:
                logger.error("Exception when calling IpcchApi->ipcch_ipcch_and_equivalent_most_recent_get: %s\n" % e)
                raise e

    def get_ipc_and_equivalent_peaks_wfp_dashboard(self, iso3: str | None = None, page: int | None = 1) -> pd.DataFrame:
        ...
        with data_bridges_client.ApiClient(self.configuration) as api_client:
            ...
            try:
                ...
                logger.info("The response of IpcchApi->ipcch_ipcch_and_equivalent_peaks_wfp_dashboard_get:\n")
                df = pd.DataFrame([item.to_dict() for item in api_response.items])
                return df
            except Exception as e:
                logger.error("Exception when calling IpcchApi->ipcch_ipcch_and_equivalent_peaks_wfp_dashboard_get: %s\n" % e)
                raise e

    def get_ipc_historical_data(self, from_date: str, to_date: str, iso3: str | None = None, page: int | None = 1) -> pd.DataFrame:
        with data_bridges_client.ApiClient(self.configuration) as api_client:
            # Create an instance of the API class
            api_instance = data_bridges_client.IpcchApi(api_client)
            env = self.env # str | Environment.   * `prod` - api.vam.wfp.org   * `dev` - dev.api.vam.wfp.org (optional)
            ...
            try:
                ...
                logger.info("The response of IpcchApi->ipcch_ipcch_historical_data_get:\n")
                df = pd.DataFrame([item.to_dict() for item in api_response.items])
                return df
            except Exception as e:
                logger.error("Exception when calling IpcchApi->ipcch_ipcch_historical_data_get: %s\n" % e)
                raise e
